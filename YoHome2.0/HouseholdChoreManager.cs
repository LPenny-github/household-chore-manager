...
namespace YoHome
{
    ...
    public class HouseholdChoreManager
    {
        ...
        string[] SearchSimilarHouseholdChore(string keyword)
        {
            ...
            // 不精確搜尋 / fuzzy search 
            similarHouseholdChore = householdChoreInformation.Where(h => h.HouseholdChoreName.Contains(keyword)).Select(n => n.HouseholdChoreName).ToArray();

            return similarHouseholdChore;
        }


        bool AreCandidatesMatchingSearchPattern(string candidateName, int searchPattern, params string[] similarHouseholdChore)
        {
